...
module AASM_StateChart
  ...
  class Chart_Renderer
    ...
    def initialize(klass, transition_table=false, config_options = {})
      ...
      @hide_enter_exit = config_options.fetch(:hide_enter_exit, false)
    ...
    def render_state(state)
      ...
      unless @hide_enter_exit
        enter_callbacks = get_callbacks(state.options, ENTER_CALLBACKS, ', ')
        exit_callbacks = get_callbacks(state.options, EXIT_CALLBACKS, ', ')
      end

      label = state_label(enter_callbacks, humanize_caps_retained(state.name), exit_callbacks) # TODO use I18n for name if there is an entry for it
    ...
    def state_label(enter_conditions, name, exit_conditions)
      ...
      enter_row = enter_conditions.present? ? enter_exit_td(humanize_caps_retained(enter_conditions),
                                                            'enter state action',
                                                            @default_config[:node_enter_label_style]) : "" # TODO I18n 'enter'

      exit_row = exit_conditions.present? ? enter_exit_td(humanize_caps_retained(exit_conditions),
                                                          'exit state action',
    ...
    def render_event(event)
      ...
      event.transitions.each do |transition|
        chunks = [humanize_caps_retained(event.name)]
    ...
    def render_callbacks(callbacks)
      callbacks.present? ? '/ ' << (humanize_caps_retained callbacks) : ''
    end
    ...
    # ActiveSupport::  humanize changes the capitalization. This does not.
    def humanize_caps_retained(underscored_word)

      result = underscored_word.to_s.dup

      ActiveSupport::Inflector.inflections.humans.each { |(rule, replacement)| break if result.sub!(rule, replacement) }
